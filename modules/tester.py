    ...
    def test_blip(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images, captions, cls_labels, clip_memory, image_path) in enumerate(self.test_dataloader):
                ...
                if batch_idx<=240:
                    for i in range(16): 
                        with open("2.txt","a", encoding='utf-8') as fp:
                            #print(i)
                            image_path=np.array_split(image_path, 16)
                            
                            print(image_path[i], file=fp)
                            print('gt:' + str(ground_truths[i]), file=fp)
                            print('reports:' + str(reports[i]), file=fp)
                elif batch_idx==241:
                    for i in range(2): 
                        with open("2.txt","a", encoding='utf-8') as fp:
                            print(image_path[i], file=fp)
                ...
                '''
                #with open("2.txt","a", encoding='utf-8') as fp:
                    #print(image_path, file=fp)
                    #print('gt:' + str(ground_truths), file=fp)
                    #print('reports:' + str(reports), file=fp)
                print(image_path)
                print('gt:' + str(ground_truths))
